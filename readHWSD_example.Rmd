...
datakey <- HWSD_mdb$HWSD_DATA %>%
  ...
  group_by(MU.GLOBAL) %>%
  summarise(SOC_top.kg_per_m2 = sum(SoilFrac * T.THICKNESS * T.BD * T.OC, na.rm=TRUE), 
          SOC_sub.kg_per_m2 = sum(SoilFrac * S.THICKNESS * S.BD * S.OC, na.rm=TRUE)
  ) %>%
  right_join(levels(hwsd_rat)[[1]], by=c( 'MU.GLOBAL' = 'ID') )
