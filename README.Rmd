...
Finally, regrid everything to a 1x1 degree grid cell via area averaging. Note that there are several ways to do this regridding and no real agreed upon method. However using a interpolation method to cross several orders of magnitude in grid size is generally not recommended. An alternate method might include aggregating the grid cell before assigning the bulk densith and soil organic carbon by assigning the dominate soil identifier to each new grid cell. You are encouraged to use your best scientific judgement in this matter.
